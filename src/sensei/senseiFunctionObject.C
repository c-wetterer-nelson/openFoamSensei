...
Foam::label Foam::functionObjects::senseiFunctionObject::expand
(
  List<string>& scripts,
...
)
{
  ...
  if (nbad)
  {
    Warning
      ...
      << " sensei scripts - check your input" << nl << nl;
  }
  ...
  if (!nscript)
  {
    Warning
      << nl
      << "No sensei scripts resolved - check your input" << nl << nl;
  }
  ...
  inputs_()
{
  if (postProcess)
  {
    ...
    FatalError << type() << " disabled in post-process mode" <<exit(FatalError);
  }

  if (!read(dict))
  {
    FatalError << type() << " with errors" << exit(FatalError);
  }
}
...
bool Foam::functionObjects::senseiFunctionObject::read(const dictionary& dict)
{
  ...
  {
    inputs_.clear();
    WarningInFunction << type() << " missing inputs" << endl;
    return false;
  }
  ...
  {
    sensei::coprocess::debug = debugLevel;
  }
  ...
  PtrList<sensei::senseiInput> newList(inputDictPtr->size());
  ...
  forAllConstIters(*inputDictPtr, iter)
  {
    ...
    auto input =
      sensei::senseiInput::New(word(iter().keyword()), time_, *subDictPtr);
...
bool Foam::functionObjects::senseiFunctionObject::execute()
{
  ...
  // sensei pipelines do not kill the simulation
  ...
  if (!adaptor_.valid())
  {
    adaptor_.reset(new sensei::coprocess());
    adaptor_().reset(outputDir_, scripts_);
  }

  sensei::dataQuery dataq(time_);
  ...
  if (nChannels)
  {
    if (sensei::coprocess::debug > 1)
    {
      Pout<< type() << ": query sensei for " << nChannels
          << " channels" << nl;
    }
  ...
  else if (sensei::coprocess::debug)
  {
...
bool Foam::functionObjects::senseiFunctionObject::end()
{
  // Only here for extra feedback
  if (log && adaptor_.valid())
  {
    Info<< type() << ": Disconnecting ParaView sensei..." << nl;
  }
...
void Foam::functionObjects::senseiFunctionObject::updateMesh(const mapPolyMesh&)
{
  for (auto& inp : inputs_)
  {
...
void Foam::functionObjects::senseiFunctionObject::movePoints(const polyMesh&)
{
  for (auto& inp : inputs_)
  {
